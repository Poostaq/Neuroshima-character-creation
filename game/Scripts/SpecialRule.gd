...
func perform_special_rule_actions(button):
	match special_rule_name:
		...
		"one_of_pack_list":
			...
			additional_data = skill_packs[button.option_button.selected]
			var chosen_skill_pack = CharacterStats.find_pack_data(additional_data["skill_pack_identifier"])
...
func undo_perform_special_rule_action():
	match special_rule_name:
		"all_skills":
			CharacterStats.flags["all_skills"] = false
		"always_bad_rep":
			CharacterStats.speciaflagsl_rules["always_bad_rep"] = false
		"bad_fame":
			pass
		"friends":
			pass
